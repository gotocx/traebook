...
导航要求：
- `index.html` 必须根据 `manifest.json` 保留每个已生成章节的链接；不能只有“开始第 1 章”而缺少后续章节入口
- `index.html` 必须在目录上方或目录卡中提供高亮的“首次进入 / 继续下一章 / 推荐下一章”CTA；下一章不能只是普通小链接
- 若当前只有 1 个已生成章节，`index.html` 仍必须把第 1 章做成高亮开始入口，不能只放在顶部导航里
- 每个非末章 `chapters/<id>.html` 必须包含指向下一章的链接；追加新章时必须回写前一章
- 每个非首章 `chapters/<id>.html` 必须包含指向上一章的链接
- 每章都必须包含返回目录链接
